#!/usr/bin/env python3
"""
Integrate SATO label overlays into HTML diagrams.
Adds clickable label buttons and modal overlay system.
"""
...
# Mapping of producto names to SVG filenames
generated = {
...
# Mapping of diagrams to products they contain
diagram_products = {
    'dark_negro.html': 'Caldo Negro',
    'dark_pescado.html': 'Caldo de Pescado',
    'dark_pollo.html': 'Caldo de Pollo',
    'dark_verduras.html': 'Caldo de Verduras',
    'dark_risotto.html': 'Caldo de Risotto',
    'dark_cremoso_iberico.html': ['Caldo de Risotto', 'Salsa Iberico'],
    'dark_cremoso_queso.html': ['Caldo de Risotto', 'Salsa de Queso'],
    'dark_cremoso_setas.html': ['Caldo de Risotto', 'Salsa de Setas'],
    'dark_cremoso_tartufato.html': ['Caldo de Risotto', 'Salsa Tartufata'],
    'dark_salsa_boletus.html': 'Salsa de Boletus',
    'dark_salsa_iberica.html': 'Salsa Iberico',
    'dark_salsa_negra.html': 'Salsa Negra',
    'dark_salsa_pescado.html': 'Salsa de Pescado',
    'dark_salsa_pollo.html': 'Salsa de Pollo',
    'dark_salsa_queso.html': 'Salsa de Queso',
    'dark_salsa_tartufata.html': 'Salsa Tartufata',
    'dark_salsa_verduras.html': 'Salsa de Verduras',
}
...
def integrate_labels_to_html(diagram_products, generated):
    ...
    for html_file, products in diagram_products.items():
        ...
        if 'SATO LABEL MODAL OVERLAY' in content:
            print(f"ℹ {html_file} already has label modal, skipping...")
            continue
        ...
        for prod_name in products:
            if prod_name not in generated:
                print(f"⚠ No generated SVG for {prod_name}")
                continue
            ...
            # Pattern: data-n="Caldo Negro"
            pattern = f'data-n="{re.escape(prod_name)}"'

            # Find all occurrences
            matches = list(re.finditer(pattern, content))
            if not matches:
                print(f"  ⚠ Product not found in {html_file}: {prod_name}")
                continue
            ...
            for match in reversed(matches):
                ...
                # Add data-etiqueta attribute and label button
                # Insert data-etiqueta before the closing > of the opening div
                div_close = node_html.find('>')
                if div_close != -1:
                    # Check if data-etiqueta already exists
                ...
                # Find the last </div> in this node
...
def main():
    print("Integrating SATO labels into HTML diagrams...\n")
